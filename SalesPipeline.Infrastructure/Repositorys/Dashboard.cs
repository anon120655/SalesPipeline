...
{    
    public class Dashboard : IDashboard
    {
        ...
        public Dashboard(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper)
        {
            _db = db;
            _repo = repo;
            _mapper = mapper;
        ...
        private async Task<IQueryable<Sale>> QueryArea(IQueryable<Sale> query, UserCustom user) // NOSONAR
        {
            ...
            else
            {
                ...
                if (!user.Role.IsAssignCenter && user.Master_Branch_RegionId != AllCountry)
                {
                    Expression<Func<Sale, bool>> orExpression = x => false;
                    //9999 เห็นทุกจังหวัดในภาค
                    if (user.Master_Branch_RegionId.HasValue && user_Areas.Contains(9999))
                    {
        ...
        public async Task<Dash_Status_TotalCustom> GetStatus_TotalById(allFilter model) // NOSONAR
        {
            ...
            if (user.Role.Code != null && !user.Role.Code.StartsWith(RoleCodes.RM, StringComparison.OrdinalIgnoreCase))
            {
                ...
                var statusTotal = await query.GroupBy(info => info.StatusSaleId)
    ...
    (user.Role.IsAssignCenter && item.StatusID == (int)StatusSaleModel.MCenterReturnLoan))
                        {
                    ...
                    dash_Status_Total.NumCusTargeNotSuccess = await user_Target_Sales.CountAsync();
                }
        ...
        public async Task<PaginationView<List<User_Target_SaleCustom>>> GetListTarget_SaleById(allFilter model) // NOSONAR
        {
            ...
            int _year = DateTime.Now.Year;
            if (!string.IsNullOrEmpty(model.year) && int.TryParse(model.year, out int year))
            {
                _year = year;
            }
