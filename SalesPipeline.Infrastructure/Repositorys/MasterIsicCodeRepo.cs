...
namespace SalesPipeline.Infrastructure.Repositorys
{
    public class MasterIsicCodeRepo : IMasterIsicCodeRepo
    {
        ...
        public async Task<PaginationView<List<Master_ISICCodeCustom>>> GetList(allFilter model)
        {
            ...
            if (!string.IsNullOrEmpty(model.businesstype) && Guid.TryParse(model.businesstype, out Guid groupId))
            {
                var gid = groupId.ToString();

                query = query.Where(x =>
                    x.GroupMaster_BusinessTypeId != null &&
                    x.GroupMaster_BusinessTypeId.Contains(gid, StringComparison.OrdinalIgnoreCase)
                );
            }
