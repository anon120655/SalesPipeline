	...
	public class MasterChains : IMasterChains
	{
		...
		public async Task<Master_ChainCustom> Create(Master_ChainCustom model)
		{
			using (var _transaction = _repo.BeginTransaction())
			{
				...
				var masterChain = new Data.Entity.Master_Chain()
				{
				...
				await _db.InsterAsync(masterChain);
				await _db.SaveAsync();

				_transaction.Commit();

				return _mapper.Map<Master_ChainCustom>(masterChain);
			}
		}

		public async Task<Master_ChainCustom> Update(Master_ChainCustom model)
		{
			using (var _transaction = _repo.BeginTransaction())
			{
				var _dateNow = DateTime.Now;

				var masterChain = await _repo.Context.Master_Chains.Where(x => x.Id == model.Id).FirstOrDefaultAsync();
				if (masterChain != null)
				{
					masterChain.UpdateDate = _dateNow;
					masterChain.UpdateBy = model.CurrentUserId;
					masterChain.Name = model.Name;
					_db.Update(masterChain);
					await _db.SaveAsync();
		...
		public async Task DeleteById(UpdateModel model)
		{
			...
			var query = await _repo.Context.Master_Chains.Where(x => x.Status != StatusModel.Delete && x.Id == id).FirstOrDefaultAsync();
			if (query != null)
			{
		...
		public async Task<PaginationView<List<Master_ChainCustom>>> GetChains(allFilter model)
		{
			...
			return new PaginationView<List<Master_ChainCustom>>()
			{
				Items = _mapper.Map<List<Master_ChainCustom>>(await items.ToListAsync()),
				Pager = pager
			};
		}
