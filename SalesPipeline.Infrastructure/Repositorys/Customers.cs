    ...
    bool? isFileUpload = false)
        {
            ...
            if (model.ValidateError == null)
                model.ValidateError = new();

            string? juristicPersonRegNumber = model.JuristicPersonRegNumber?.Trim();
            string[] prefixAmphurToRemove = { "กิ่ง", "เขต" };

            // -------------------------
            // Validate: เลขนิติบุคคล
            // -------------------------
            if (juristicPersonRegNumber == null || juristicPersonRegNumber.Length != 13)
            {
                AddErrorAndThrowIfNeeded(model, "เลขทะเบียนนิติบุคคลไม่ถูกต้อง", isThrow);
            }

            // -------------------------
            // Validate: เช็คซ้ำในระบบ
            // -------------------------
            if (model.IsRePurpose != true)
            {
                var customer = await _repo.Context.Customers
                    .Where(x => x.JuristicPersonRegNumber == juristicPersonRegNumber)
                    .FirstOrDefaultAsync();

                if (customer != null)
                {
                    model.Id = customer.Id;
                    AddErrorAndThrowIfNeeded(model,
                        $"มีเลขทะเบียนนิติบุคคล {customer.JuristicPersonRegNumber} ในระบบแล้ว",
                        isThrow);
                }
                ...
                if (juristicPersonRegNumber != null)
                {
                    ...
                    {
                        AddErrorAndThrowIfNeeded(model,
                            juristicNumber.Message ?? string.Empty,
                            isThrow);
                    }
                }
            }
            ...
            // -------------------------
            // Validate: วันที่ห้ามเป็นอนาคต
            // -------------------------
            if (model.DateContact.HasValue &&
                model.DateContact.Value.Date > DateTime.Now.Date)
            {
                AddErrorAndThrowIfNeeded(model,
                    "วันที่เข้ามาติดต่อ ต้องไม่มากกว่าวันที่ปัจจุบัน",
                    isThrow);
            }
            ...
            if (string.IsNullOrEmpty(model.CompanyName))
                AddErrorAndThrowIfNeeded(model, "ระบุ ชื่อบริษัท", isThrow);

            if (string.IsNullOrEmpty(model.HouseNo))
                AddErrorAndThrowIfNeeded(model, "ระบุ บ้านเลขที่", isThrow);

            // -------------------------
            // Set Master Data เมื่อระบุ
            // -------------------------
            if (isSetMaster == true)
            {
                // Region
                ...
                else if (!string.IsNullOrEmpty(model.Master_BusinessTypeName))
                {
                ...
                else if (!string.IsNullOrEmpty(model.Master_BusinessSizeName))
                {
                ...
                else if (!string.IsNullOrEmpty(model.Master_TSICName))
                {
                ...
                else if (!string.IsNullOrEmpty(model.ProvinceName))
                {
                ...
                else if (!string.IsNullOrEmpty(model.AmphurName))
                {
                ...
                else if (!string.IsNullOrEmpty(model.TambolName))
                {
            ...
            // -------------------------
            // Province / Amphur / Tambol ต้องมีค่า
            // -------------------------
            if (!model.ProvinceId.HasValue)
                AddErrorAndThrowIfNeeded(model, "ระบุ จังหวัด", isThrow);

            if (!model.AmphurId.HasValue)
                AddErrorAndThrowIfNeeded(model, "ระบุ เขต/อำเภอ", isThrow);

            if (!model.TambolId.HasValue)
                AddErrorAndThrowIfNeeded(model, "ระบุ แขวง/ตำบล", isThrow);
        ...
        private void AddErrorAndThrowIfNeeded(CustomerCustom model, string message, bool isThrow)
        {
            ...
            model.ValidateError.Add(message);

            if (isThrow)
            {
                // NOSONAR: throw ตามความต้องการ ไม่ใช่ unreachable
            ...
            using (var _transaction = _repo.BeginTransaction())
            {
                ...
                if (customer != null)
                {
                    ...
                    if (user.LevelId >= 10 && user.LevelId <= 12)
                    {
                        ...
                        {
                            if (await _repo.Context.Customers.AnyAsync(x => x.Id != model.Id && x.JuristicPersonRegNumber == model.JuristicPersonRegNumber))
                    ...
                    var customer_Committees = await _repo.Context.Customer_Committees.Where(x => x.CustomerId == model.Id).ToListAsync();
                    ...
                    var customer_Shareholders = await _repo.Context.Customer_Shareholders.Where(x => x.CustomerId == model.Id).ToListAsync();
