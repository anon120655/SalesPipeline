...
namespace SalesPipeline.ViewModels
{
	public class AssignmentCenterViewModel
	{
		...
		public async Task<ResultModel<PaginationView<List<Assignment_CenterCustom>>>> GetListAutoAssign(allFilter model)
		{
			try
			{
				string tokenJwt = await _authorizeViewModel.GetAccessToken();
				string dataJson = JsonConvert.SerializeObject(model);
				var content = await _httpClient.PostAsync($"/v1/AssignmentCenter/GetListAutoAssign", dataJson, token: tokenJwt);
