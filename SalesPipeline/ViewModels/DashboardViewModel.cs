...
namespace SalesPipeline.ViewModels
{
	public class DashboardViewModel
	{
		...
		public async Task<ResultModel<PaginationView<List<User_Target_SaleCustom>>>?> GetListTarget_SaleById(allFilter model)
		{
			try
			{
				string tokenJwt = await _authorizeViewModel.GetAccessToken();
				string dataJson = JsonConvert.SerializeObject(model);
				var content = await _httpClient.PostAsync($"/v1/Dashboard/GetListTarget_SaleById", dataJson, token: tokenJwt);
				var dataMap = JsonConvert.DeserializeObject<PaginationView<List<User_Target_SaleCustom>>>(content);
				return new ResultModel<PaginationView<List<User_Target_SaleCustom>>>()
				{
					Data = dataMap
				};
			}
		...
		public async Task<ResultModel<List<GroupByModel>>?> GetAvgTopBar(allFilter model)
		{
			try
			{
				string tokenJwt = await _authorizeViewModel.GetAccessToken();
				string dataJson = JsonConvert.SerializeObject(model);
				var content = await _httpClient.PostAsync($"/v1/Dashboard/GetAvgTopBar", dataJson, token: tokenJwt);
		...
		public async Task<ResultModel<List<GroupByModel>>?> GetAvgRegionMonth12Bar(allFilter model)
		{
			try
			{
				string tokenJwt = await _authorizeViewModel.GetAccessToken();
				string dataJson = JsonConvert.SerializeObject(model);
				var content = await _httpClient.PostAsync($"/v1/Dashboard/GetAvgRegionMonth12Bar", dataJson, token: tokenJwt);
				var dataMap = JsonConvert.DeserializeObject<List<GroupByModel>>(content);
				return new ResultModel<List<GroupByModel>>()
				{
					Data = dataMap
