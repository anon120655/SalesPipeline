...
namespace SalesPipeline.ViewModels
{
    public class AuthorizeViewModel : AuthenticationStateProvider
    {
        ...
        public async Task<String> GetAccessToken()
        {
            ...
            {
                // Swallow exception intentionally. Returning empty token is acceptable in this context.
            }
            return access_token;
        }
        ...
        public async Task<ResultModel<LoginResponseModel>> LoginAsync(LoginRequestModel user)
        {
            try
            {
                ...
                if (!userInDatabase.Status || userInDatabase.Data == null) 
                    throw new InvalidOperationException(userInDatabase.errorMessage);
                ...
                await _protectedLocalStorage.SetAsync(IdentityKey, JsonConvert.SerializeObject(userInDatabase.Data));
        ...
        private static  ClaimsIdentity CreateIdentityFromUser(LoginResponseModel user)
        {
        ...
        public async Task<ResultModel<LoginResponseModel>> Authenticate(LoginRequestModel model)
        {
            try
            {
                    ...
                    if (data != null)
                    {
                        ...
                        else
                        {
                            if (data.iauthen != null)
                            {
                                ...
                                {
                                    throw new InvalidOperationException($"iAuth : {data.iauthen.response_message}");
                                }
                                else
                                {
                                    throw new ExceptionCustom($"เชื่อมต่อ iAuth ไม่สำเร็จ กรุณาติดต่อผู้ดูแลระบบ");
                ...
                else
                {
                    ...
                    {
                        throw new InvalidOperationException(GeneralUtils.MapErrorModel(await response.Content.ReadAsStringAsync()));
                    }
                }

                throw new InvalidOperationException($"{response.StatusCode} กรุณาติดต่อผู้ดูแลระบบ");
            }
            catch (Exception ex)
            {
                return new ResultModel<LoginResponseModel>
