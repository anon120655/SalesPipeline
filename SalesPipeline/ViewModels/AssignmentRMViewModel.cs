...
namespace SalesPipeline.ViewModels
{
	public class AssignmentRMViewModel
	{
		...
		public async Task<ResultModel<Boolean>> Return(ReturnModel model)
		{
			try
			{
				string tokenJwt = await _authorizeViewModel.GetAccessToken();
				string dataJson = JsonConvert.SerializeObject(model);
				var content = await _httpClient.PostAsync($"/v1/AssignmentRM/Return", dataJson, token: tokenJwt);
		...
		public async Task<ResultModel<bool>?> CreateAssignmentRMAll()
		{
			try
			{
				var content = await _httpClient.GetAsync($"/v1/AssignmentRM/CreateAssignmentRMAll");
