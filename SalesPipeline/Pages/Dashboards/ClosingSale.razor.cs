...
namespace SalesPipeline.Pages.Dashboards
{
	public partial class ClosingSale
	{
		...
		{
			var data = await _exportViewModel.ExcelClosingSale(filter);
			if (data != null && data.Status && data.Data != null)
			{
				await _jsRuntimes.InvokeAsync<object>("saveAsFile", "รายงานลูกค้าที่ปิดการขาย.xlsx", Convert.ToBase64String(data.Data));
			}
			else
			{
				_errorMessage = data?.errorMessage;
