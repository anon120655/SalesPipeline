...
namespace SalesPipeline.Pages.Dashboards
{
	public partial class TopSales
    {
		...
		{
			var data = await _exportViewModel.ExcelTopSales(filter);
			if (data != null && data.Status && data.Data != null)
			{
				await _jsRuntimes.InvokeAsync<object>("saveAsFile", "รายงาน 10 อันดับ จังหวัดยอดขายสูงสุด.xlsx", Convert.ToBase64String(data.Data));
			}
			else
			{
				_errorMessage = data?.errorMessage;
