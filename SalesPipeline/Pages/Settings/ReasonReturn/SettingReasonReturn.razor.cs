...
{
	public partial class SettingReasonReturn
	{
		...
		private List<Master_ReasonReturnCustom>? Items;
		public Pager? Pager;
		...
		protected async Task SetModel()
        {
			...
			if (data != null && data.Status)
			{
				...
				if (Pager != null)
				{
					Pager.UrlAction = "/setting/reason";
				}
			}
			else
			{
		...
		protected async Task Delete(string id)
		{
			...
			var data = await _masterViewModel.DeleteReasonReturnById(new UpdateModel() { id = id, userid = UserInfo.Id });
		...
		protected async Task StatusChanged(ChangeEventArgs e, Guid id)
		{
			...
			{
				var data = await _masterViewModel.UpdateStatusReasonReturnById(new UpdateModel() { id = id.ToString(), userid = UserInfo.Id, value = val.ToString() });
				if (data != null && !data.Status && !String.IsNullOrEmpty(data.errorMessage))
				{
					_errorMessage = data?.errorMessage;
					_utilsViewModel.AlertWarning(_errorMessage);
