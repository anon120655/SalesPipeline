...
{
	public partial class SettingYieldUpload
	{
		...
		List<Master_YieldCustom>? YieldList = new();
			...
			else
			{
				try
				{
					using (var stream = file.OpenReadStream(TenMegaBytes))
					{
						...
						IFormFile files = new FormFile(ms, 0, bytefile.Length, "name", "YieldImport.xlsx");
						...
						if (files.Length > 0)
						{
							...
							using (var streamread = new FileStream(fullPath, FileMode.Create))
							{
								...
								for (var rowIndex = 1; rowIndex <= sheet.LastRowNum; rowIndex++)
								{
									...
									if (header_list.TryGetValue("ชื่อผลผลิต", out cellIndex))
									{
										Name = row.GetCell(cellIndex).ToString();
									}

									if (YieldList == null) YieldList = new();

									if (string.IsNullOrEmpty(Name))
										throw new Exception("ชื่อผลผลิต ไม่ครบถ้วน");
								...
								{
									var response = await _masterViewModel.ValidateUploadYield(YieldList);

									if (response.Status)
									{
										YieldList = response.Data?.OrderBy(x => x.IsValidate == true).ToList();
									}
									else
									{
										HideLoading();
		...
		protected async Task Save()
		{
			...
			if (YieldList == null || YieldList.Count(x => x.IsValidate == true) == 0)
			{
			...
			else
			{
				...
				foreach (var item in YieldList.Where(x => x.IsValidate == true).ToList())
				{
					var resultModel = new ResultImport();
					resultModel.Name = item.Name;

					var response = await _masterViewModel.CreateYield(item);
					if (!response.Status)
					{
