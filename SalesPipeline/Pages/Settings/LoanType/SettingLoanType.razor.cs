...
namespace SalesPipeline.Pages.Settings.LoanType
{
	public partial class SettingLoanType
	{
		...
		private List<Master_LoanTypeCustom>? Items;
		public Pager? Pager;
		...
		protected async Task SetModel(bool resetPage = true)
		{
			...
			if (data != null && data.Status)
			{
				...
				if (Pager != null)
				{
					Pager.UrlAction = "/setting/loantype";
				}
			}
			else
			{
		...
		protected async Task Delete(string id)
		{
			...
			var data = await _masterViewModel.DeleteLoanTypeById(new UpdateModel() { id = id, userid = UserInfo.Id });
		...
		protected async Task StatusChanged(ChangeEventArgs e, Guid id)
		{
			...
			{
				var data = await _masterViewModel.UpdateStatusLoanTypeById(new UpdateModel() { id = id.ToString(), userid = UserInfo.Id, value = val.ToString() });
				if (data != null && !data.Status && !String.IsNullOrEmpty(data.errorMessage))
				{
					_errorMessage = data?.errorMessage;
					_utilsViewModel.AlertWarning(_errorMessage);
